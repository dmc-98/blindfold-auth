...
test("createTestAuth seeds users + roles and supports login", async () => {
  const t = await createTestAuth({
    ...
    users: [{ email: "admin@x.com", password: "testing-password-123!", roles: ["operator"] }]
  });

  const login = await t.login("admin@x.com", "testing-password-123!");
...
test("assertCan / assertCannot reflect RBAC permissions", async () => {
  const t = await createTestAuth({
    ...
    users: [{ email: "r@x.com", password: "testing-password-123!", roles: ["reader"] }]
  });
...
test("mintSession issues a session without a password round-trip", async () => {
  const t = await createTestAuth({
    users: [{ email: "u@x.com", password: "testing-password-123!" }]
  });
...
test("unknown role reference is rejected loudly", async () => {
  await assert.rejects(
    () =>
      createTestAuth({
        users: [{ email: "u@x.com", password: "testing-password-123!", roles: ["ghost"] }]
      }),
    /unknown role "ghost"/
  );
});
