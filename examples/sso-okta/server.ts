/**
 * server.ts — Okta OIDC SSO example for Blindfold Auth.
 *
 * Prerequisites:
 *   1. Copy .env.example → .env and fill in credentials
 *   2. Run `npm run setup` once to register the Okta provider + binding
 *   3. Run `npm start` (or `npm run dev` for ts-node hot-reload)
 *
 * See docs/sso/okta.md for the full step-by-step guide, including:
 *   - Creating the Okta OIDC app
 *   - Validating IdP metadata with `blindfold sso doctor`
 *   - JIT user provisioning and SAML 2.0 variant
 */
...
<head>
  <meta charset="utf-8" />
  <title>Blindfold + Okta OIDC demo</title>
  <style>
    ...
    button { background: #0052cc; color: #fff; border: none; padding: 10px 20px; border-radius: 6px; font-size: 14px; cursor: pointer; width: 100%; }
    button:hover { background: #003d99; }
    .note { font-size: 13px; color: #666; margin-top: 12px; }
  </style>
</head>
<body>
  <h2>Blindfold + Okta OIDC</h2>
  <p>Enter your work email to sign in via Okta (optional — leave blank to skip domain routing).</p>
  <form method="POST" action="/auth/sso/start">
    <input name="email" type="email" placeholder="you@yourcompany.okta.com" />
    <button type="submit">Sign in with Okta →</button>
  </form>
  <p class="note">
    This demo uses <a href="https://github.com/dmc-98/blindfold-auth">Blindfold Auth</a>
    for SSO. See <code>docs/sso/okta.md</code> for the full recipe.
  </p>
</body>
</html>`);
});
...
app.get("/auth/sso/oidc/callback", async (req, res) => {
  try {
    ...
    // Here we use a simple HttpOnly cookie.
    ...
    res
      .status(401)
      .send(`Authentication failed: ${(err as Error).message}. <a href="/">Try again</a>`);
  }
});
...
<head>
  <meta charset="utf-8" />
  <title>Dashboard — Blindfold + Okta</title>
  <style>
...
app.listen(PORT, () => {
  ...
  console.log(`SSO callback URL (register this in Okta): http://localhost:${PORT}/auth/sso/oidc/callback`);
});
