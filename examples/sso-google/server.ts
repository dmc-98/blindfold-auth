/**
 * server.ts — Google Workspace OIDC SSO example for Blindfold Auth.
 *
 * Prerequisites:
 *   1. Copy .env.example → .env and fill in credentials
 *   2. Run `npm run setup` once to register the Google provider + binding
 *   3. Run `npm start` (or `npm run dev` for ts-node hot-reload)
 *
 * See docs/sso/google.md for the full step-by-step guide, including:
 *   - Creating the OAuth client in Google Cloud Console
 *   - Validating IdP metadata with `blindfold sso doctor`
 *   - Group-based role mapping options and the SAML 2.0 variant
 */
...
<head>
  <meta charset="utf-8" />
  <title>Blindfold + Google Workspace OIDC demo</title>
  <style>
    ...
    button { background: #1a73e8; color: #fff; border: none; padding: 10px 20px; border-radius: 6px; font-size: 14px; cursor: pointer; width: 100%; }
    button:hover { background: #1558b0; }
    .note { font-size: 13px; color: #666; margin-top: 12px; }
  </style>
</head>
<body>
  <h2>Blindfold + Google Workspace</h2>
  <p>Enter your work email to sign in via Google (optional — leave blank to skip domain routing).</p>
  ...
  <p class="note">
    ...
    for SSO. See <code>docs/sso/google.md</code> for the full recipe.
  </p>
</body>
</html>`);
});
...
// → 302 redirect to Google login (accounts.google.com)
...
app.get("/auth/sso/oidc/callback", async (req, res) => {
  try {
    ...
    // Common Google errors: redirect_uri_mismatch, org_internal, admin_policy_enforced
...
<head>
  <meta charset="utf-8" />
  <title>Dashboard — Blindfold + Google Workspace</title>
  <style>
...
<body>
  <h2>✓ Signed in via Google Workspace</h2>
  <p>Session details:</p>
...
app.get("/logout", (_req, res) => {
  res.clearCookie("session");
  // Note: Google has no OIDC RP-initiated logout endpoint; clearing the local
  // session is the standard pattern. To force account re-selection on next
  // login, Google supports prompt=select_account on the authorize request.
  res.redirect("/");
});
...
app.listen(PORT, () => {
  ...
  console.log(
    `SSO callback URL (register in Google Cloud Console): http://localhost:${PORT}/auth/sso/oidc/callback`
  );
});
